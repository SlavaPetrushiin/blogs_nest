...
export class AuthService {
  ...
  async confirmCode(code: string) {
    ...
    const isUpdateConfirm = await this.usersService.updateConfirmationStatus(
      user.id,
    );
  ...
  async confirmResending(email: string) {
    ...
    const updatedClient = await this.usersService.updateConfirmationCode(
      user.id,
      newCode,
      newExpirationData,
    );
  ...
  async passwordRecovery(email: string) {
    ...
    const isCreatedRecovery =
      await this.passwordRecoveryRepository.createOrUpdatePasswordRecovery(
        recoveryCode,
        email,
        dateExpired,
      );
  ...
  async updatePassword(newPassword: string, recoveryCode: string) {
    const foundedRecoveryObject =
      await this.passwordRecoveryRepository.getRecoveryPassword(recoveryCode);
    ...
    const isUpdatedPassword = await this.usersService.updatePassword(
      user.id,
      passwordHash,
    );
  ...
  async updateRefreshToken(user: {
    ...
    const foundedSession = await this.authRepository.getSession(
      convertJwtPayloadSecondsToIsoDate(user.iat),
      id,
      deviceId,
    );
    ...
    if (!foundedSession) {
      throw new BadRequestException(
        getArrayErrors('email', 'Пользователя не найден'),
      );
    }

    const accessToken = this.jwtService.sign(
      { id },
      { secret: process.env.JWT_ACCESS_SECRET, expiresIn: EXPIRES_ACCESS_TIME },
    );
